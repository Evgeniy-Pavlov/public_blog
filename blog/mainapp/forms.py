...
from .models import UserBase, Article, CommentsArticle, LikesArticle, News, LikesNews, CommentsNews
...
class UpdateUserForm(forms.ModelForm):
    ...
    company = forms.CharField(max_length=20, widget=forms.TextInput(attrs={'class': 'form-control'}), label='Название компании')
...
class NewsForm(forms.ModelForm):
    ...
    text = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'style': 'width: 850px;'}), label='Текст новости', required=True)
    images = MultiMediaField(widget = MultiUploadMetaInput(attrs={'class': 'form-control', 'style': 'width: 850px;'}), label='Вложения')
...
class CommentsNewsCreateForm(forms.ModelForm):
    """Форма создания комментария."""
